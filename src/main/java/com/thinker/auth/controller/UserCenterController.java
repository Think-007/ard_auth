...
public class UserCenterController {
	...
	public ProcessResult uploadPic(MultipartFile file,
		...
		try {
			...
			Thumbnails.of(picfile).scale(1f).outputQuality(0.25f)
					.toFile(picfile);

			Thumbnails.of(picfile).size(80, 80).keepAspectRatio(false)
					.toFile(micPicFile);
		} catch (Exception e) {
			e.printStackTrace();
		}
